...
function cargar_info(no){
  ...
  html+= '<div class="infohotel"> <div class="row"> <div class="col-md-6 col-xs-12"><h2>' + name + '</h2>';
...
function load_git(){
    ...
    console.log("han pulsado load git");
...
function save_git(){
  ...
  console.log(token + repo + usuario);
}

function save(){
  console.log("han pulsado save");
  $("#formSave").show()
}

function actualizar_list3(){
    console.log("actualizamos list")
    console.log(coleccion_selecionada)
    console.log(colecciones[coleccion_selecionada]);
...
function show_hotel(){
  ...
  actualizar_list3()
  console.log("pinchan en una coleccion para mostrar los hoteles" + id)
...
function show_accomodation(){
  ...
  if (accomodation.extradata.categorias.categoria.subcategorias != undefined){
   ...
   for (var i = 0; i < markers.length; i++) {
    ...
    if (!markerexists){
      markers.push(marker);
      marker.addTo(map).bindPopup('<a no="'+ no +'" href="' + url + '">' + name + '</a><br/>' + "<a href='#desc'>+ info</a><button onclick='del("+lat+","+lon+")'>Eliminar</button>")
